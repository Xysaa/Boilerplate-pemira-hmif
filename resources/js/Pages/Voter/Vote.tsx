...
import { CheckCircle2, ChevronRight, User2, Shield, Loader2, ArrowLeft } from 'lucide-react';
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div>
              <p className={cn('font-bold text-base leading-tight', selected ? 'text-white' : 'text-emerald-200/80')}>
                {candidate.name}
              </p>
              {candidate.vice_name && (
...
export default function VotePage({ voter, session, candidates }: VotePageProps) {
  ...
  const { post, processing } = useForm({ candidate_id: selectedId });

  const selectedCandidate = candidates.find(c => c.id === selectedId);

  const handleConfirm = () => {
    if (!selectedId) return;
    post(route('voter.vote.submit'), {
      data: { candidate_id: selectedId },
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-1">
          <Link href={route('voter.dashboard')}>
            <Button variant="ghost" size="sm" className="text-emerald-600 hover:text-emerald-400 -ml-2 mb-1 gap-1">
          ...
          <p className="text-emerald-500/50 text-sm">Pilih salah satu kandidat di bawah ini</p>
        </motion.div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {selectedCandidate.vice_name && (
                    <p className="text-emerald-500/50 text-xs">& {selectedCandidate.vice_name}</p>
                  )}
                </div>
              </div>
            </div>
